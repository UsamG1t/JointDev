...
def addmon_answer(locale, code, name=None, hp=None, replace_check=None):
    ...
    response.append(ngettexts(locale,
                             'Added monster {} with {} hp',
                             'Added monster {} with {} hp',
                             hp).format(name, hp))
    if replace_check:
        response.append(gettexts(locale, replace_check))
...
def attack_answer(locale, name, code, dmg=None, hp=None):
    ...
    if code == '1':
        response.append(gettexts(locale, 'No {} here').format(name))
        return '\n'.join(response)
    ...
    if hp != '0':
        ...
        response.append(gettexts(locale, '{} died').format(name))

    return '\n'.join(response)


def ngettexts(locale, *text):
    return LOCALES[locale].ngettext(*text)


def gettexts(locale, *text):
...
class Game:
    ...
    def locale(self, player, locale):
        ...
        response.append(gettexts(locale, 'Set up locale: {}').format('.'.join(locale)))
                    ...
                    case 'register':
                        ...
                        for user, user_queue in users.items():
                            if user_queue is not my_queue:
                                await user_queue.put(gettexts(users_info[user].locale,
